...
def generateUnregisterMessage(
    ...
    if all:
        return Unregister(
            message_id=message_id, capability_id=None, fin_id=None, all=True
        )
    if capability_id:
        return Unregister(
            message_id=message_id, capability_id=capability_id, fin_id=None, all=False
        )
    if fin_id:
        return Unregister(
            message_id=message_id, capability_id=None, fin_id=fin_id, all=False
        )
...
def generateResultStructureMessage(
    state: str, context: Context, variables: dict[str, Variable]
) -> ResultStructure:
    return ResultStructure(state=state, context=context, variables=variables)


def generateResultMessage(
    result: ResultStructure, meta: Meta, message_id: str = None
) -> Result:
    message_id = message_id
    if not message_id:
        message_id = str(uuid1())
...
def generateVariableMessage(
    type: VariableTypeEnum, description: str, value: str, constant: bool, external: bool
) -> Variable:
    return Variable(
        type=type,
        description=description,
...
def generateAuthenticationInformationMessage(
    type: AuthTypeEnum,
    name: str = None,
    description: str = None,
    authentication_info_extenstion: dict[str, str] = None,
) -> AuthenticationInformation:
...
def generateCommandSubStructureMessage(
    command: str,
    context: Context,
    variables: dict[str, Variable],
    authentication: dict[str, AuthenticationInformation] = None,
) -> CommandSubStructure:
...
def generateCommandMessage(
    command: CommandSubStructure, meta: Meta, message_id: str = None
) -> Command:
    message_id = message_id
    if not message_id:
        message_id = str(uuid1())
